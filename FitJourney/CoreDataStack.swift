...
class CoreDataStack {
	...
	static let moduleName = "FitJourney"
	
	lazy var persistentStoreCoordinator: NSPersistentStoreCoordinator? = {
		// The managed object model for the application. This property is not optional. It is a fatal error for the application not to be able to find and load its model.
		
		if let modelURL = NSBundle.mainBundle().URLForResource("FitJourney", withExtension: "momd") {
			...
			if let managedModel = NSManagedObjectModel(contentsOfURL: modelURL) {
				...
				// The directory the application uses to store the Core Data store file. This code uses a directory named "com.flashbolt.FitJourney" in the application's documents Application Support directory.
